...
(function () {
    ...
    .controller('elecDemandChartController', elecDemandChartController);

    elecDemandChartController.$inject = ['$scope', '$q', '$timeout', 'Colors', '$http', 'dateUtils'];
    function elecDemandChartController($scope, $q, $timeout, Colors, $http, dateUtils) {
        ...
        $scope.$on('Elec_demandData', function (event,data){
            vm.demandData = data;
            activate(Linechart.Mode.RAW);
        });
        $scope.$on('elecUsage_datepick', function (event,data){
            vm.demandData = data;
            activate(Linechart.Mode.RAW);
        });
        ...
        function activate(mode) {
            ...
            drawChart(document.getElementById("elecDemand").getContext('2d'), config);
        }
        ...
        function buildChartDataSet(mode) {
            ...
            switch (mode) {
                case Linechart.Mode.HOUR:
                    usageValue = vm.demandData.data.valueDataHour;
                    preValue = vm.demandData.pre.valueDataHour;
                    //cblValue = vm.SiteData.cblDataHour;
                    break;
                case Linechart.Mode.HALF:
                    usageValue = vm.demandData.data.valueDataHalfHour;
                    preValue = vm.demandData.pre.valueDataHalfHour;
                    //cblValue = vm.SiteData.cblDataHalf;
                    break;
                case Linechart.Mode.QUARTER:
                    usageValue = vm.demandData.data.valueDataQuaterHour;
                    preValue = vm.demandData.pre.valueDataQuaterHour;
                    //cblValue = vm.SiteData.cblDataQuater;
                    break;
                case Linechart.Mode.RAW:
                    usageValue = vm.demandData.data.raw;
                    preValue = vm.demandData.pre.raw;
                    break;
            }
        ...
        function drawChart(objChart, data) {
            ...
            elecDemand = new Chart(objChart, data);
        }
    // }
