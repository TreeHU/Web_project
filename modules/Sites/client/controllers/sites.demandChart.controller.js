(function () {
    ...
    .controller('demandChartController', demandChartController);

    demandChartController.$inject = ['$scope', '$q', '$timeout', 'Colors', '$http', 'dateUtils'];
    function demandChartController($scope, $q, $timeout, Colors, $http, dateUtils) {

        var vm = this;
        var dataset_usage = [];
        var config;
        var sitesDemandChart;
        ...
        $scope.$on('demandData', function (event, data) {   
            vm.SiteData = data;
            activate(Linechart.Mode.RAW);
        });
        $scope.$on('usage_datepick',function(event,data){
            vm.SiteData = data;
            activate(Linechart.Mode.RAW);
        });
        function activate(mode) {    
            ...
            drawChart(document.getElementById("sitesDemandChart").getContext('2d'), config);
        }
        ...
        function drawChart(objChart, data) {
            if (sitesDemandChart != null) {
                sitesDemandChart.destroy();
            }
            sitesDemandChart = new Chart(objChart, data);
        }

        $scope.raw = function () {
            if ($scope.lastChartState !== Linechart.Mode.RAW) {
                activate(Linechart.Mode.RAW);
                sitesDemandChart.update();
            }
        };
