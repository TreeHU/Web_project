(function () {
    ...
    angular
        ...
        .controller('TotalSiteV2Controller', TotalSiteV2Controller);

    TotalSiteV2Controller.$inject = ['$scope', '$http','$location', 'dateUtils', 'FacilityTotalUsage','totalSite','facilityInfo','sensorlist','sessionstoarge', 'predict'];
    function TotalSiteV2Controller($scope, $http, $location, dateUtils, FacilityTotalUsage, totalSite, facilityInfo,sensorlist,sessionstoarge, predict) {
                            ...
                            totalSite.EachFacilitySupply(date).then(function(total_supply){//모든 시설, 각각의 시설별 금일 생산량
                                totalSite.supplySelectedFacility(facilityList,date).then(function(supply){//금일,어제 생산량
                                    totalSite.usageSelectedFacilityweek(facilityList,date).then(function(usage){//금일,어제 사용량
                                        ...
                                        //1시간단위 생산량 예측데이터
                                        // $http.get('/api/preSupplyHourV2_Today').then(function(result){
