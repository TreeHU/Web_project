...
(function () {
    ...
    .controller('ElecStackedChartController', ElecStackedChartController);

    ElecStackedChartController.$inject = ['$scope', '$q', '$timeout', 'Colors', '$http', 'dateUtils'];
    function ElecStackedChartController($scope, $q, $timeout, Colors, $http, dateUtils) {
        ...
        Chart.Tooltip.positioners.myCustomPosition = function(unused, position) {
            return { x: position.x, y: 250 }; // HARDCODING VALUES
         }

        $scope.$on('selectedData', function (event, data) {
            vm.TsData = data.EachFacilityUsage;
            //각 시설별 사용량만 담을 변수 선언
        ...
        function buildChartDataSet(mode) {
            ...
            if (mode !== Linechart.Mode.RAW) {
                ...
                return dataSet;//dataSet : [dataObj{raw:{}}]
            }
        }
        ...
        function bulidChartOptions(mode) {
            ...
            function setDataSets(obj){
                ...
                for(var i=1; i<Object.keys(obj).length; i++){
                    var dic = new Object();
                    dic.label = Object.keys(obj)[i];//여기이름바꾸자
                    dic.data = [];
                    dic.spanGaps = true;
