...
(function () {
    ...
    .controller('StackedSupplyChartController', StackedSupplyChartController);

    StackedSupplyChartController.$inject = ['$scope', '$q', '$timeout', 'Colors', '$http', 'dateUtils'];
    function StackedSupplyChartController($scope, $q, $timeout, Colors, $http, dateUtils) {
        ...
        $scope.$on('SdData', function (event, data) {
            vm.SdData = data.uses_supply;
            vm.PreD = data.pre_output;
        ...
        function activate(mode) {
            ...
            drawChart(document.getElementById("supplychart").getContext('2d'), config);
        }
        ...
        function bulidChartOptions(mode) {
            ...
            if (mode === Linechart.Mode.RAW) {
                ...
                //x축 vm.dataSet[0].xaxis 로 (셋 다 똑같음)
                config.data.labels = vm.dataSet[0].xaxis;
                config.data.datasets[0].data = vm.dataSet[0].commercial;
                config.data.datasets[1].data = vm.dataSet[0].residential;
                config.data.datasets[2].data = vm.dataSet[0].edu;
        ...
        // var supplychart = null;
        function drawChart(objChart, data) {
            if (supplychart != null) {
                supplychart.destroy();
            }
            supplychart = new Chart(objChart, data);
        }
